...
from AppKit import NSWorkspace, NSScreen, NSWorkspaceDesktopImageScalingKey, NSImageScaleProportionallyUpOrDown, NSWorkspaceDesktopImageAllowClippingKey
...
# Some globals
DIR = os.getcwd()
SUBREDDITS = ["museum", "ColorizedHistory",
				"MacroPorn", "FuturePorn", "spaceporn",
				"historyporn", "ImaginaryLandscapes", "movieposterporn",
				"RetroFuturism", "imaginaryhistory",
				"ImaginaryTechnology", "auroraporn", "LandscapeAstro",
				"StarshipPorn", "AlbumArtPorn", "ArtPorn", "OrganizationPorn",
				"MapPorn", "FossilPorn", "desertporn", "seaporn", "waterporn",
				"WeatherPorn", "ViewPorn", "ExposurePorn", "HellscapePorn",
				"GeekPorn", "AdrenalinePorn", "ClimbingPorn", "BonsaiPorn",
				"MushroomPorn", "geologyporn", "minimalist_art", "romanticism",
                "ancient_art", "Medievalart", "painting", "MuseumPorn", "SculpturePorn",
                "BattlePaintings", "ArtefactPorn"]
...
def get_reddit():
    ...
    r = praw.Reddit(client_id=REDDIT_CLIENT_ID,
                     ...
                     username=REDDIT_UN,
                     password=REDDIT_PW)
    good_url = [x.url for x in r.subreddit(SUB).top('month', limit=3) if "jpg" or "png" in x][0]
...
def change_desktop_background(file):
    ...
    options = { NSWorkspaceDesktopImageScalingKey: NSImageScaleProportionallyUpOrDown, NSWorkspaceDesktopImageAllowClippingKey: True }
